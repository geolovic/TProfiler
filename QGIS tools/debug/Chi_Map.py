...
Distance = 250
Output_shapefile = "../../test/data/qgis_test.shp"
Threshold = 1000
Units = "CELL"
Basins = True
...
def main(dem, fac, distance, out_shapefile, umbral, units, basins, basin_shp, main_ch, head_shp, thetaref, reg_points, smooth):
    ...
    if basin_shp:
        ...
        for feat in layer:
            ...
            if len(basin_heads > 0):
                out_profiles.extend(p.get_profiles(fac, dem, basin_heads, tributaries=True))
...
main(DEM, Flow_Accumulation, Output_shapefile, Threshold, units=Units, distance=Distance, basin_shp=Basins,
     main_ch=Main_channels)
